...
Turn_durations=5:13;
for ii=1:length(Turn_durations)
    clearvars -except ii Turn_durations coverage_perc Dist_Exit
    ...
    max_area=Dis_total*2*rc+pi*(rc)^2;                          %m^2    
    ...
    while top_level_E>0
        ...
        for count=1:ct
            ...
            y0=h(1,2);
            theta0=h(1,4);
            min_cost=@(omega_dot)RHC_opt_mod_algo(omega_dot,dt,tspan,x0,y0,theta0,p_exit,E_batt_remain,Power_req,boundaryx,boundaryy,covered_area_x,covered_area_y,xc',yc',Velocity,epsilon,gravity,rho,W_area,CD,Cd_i,eta);
            ...
            c_path(1:size(calc_path,1),1)=alt;
            sel_path=[calc_path(:,1) calc_path(:,2) c_path calc_path(:,3)];
    ...
    coverage_perc(:,ii)=top_level_area_covered/Area*100;
    Dist_Exit(:,ii)=sqrt(sum((top_PATH(end,1:2)-p_exit(1:2)).^2));
